...
//	extern __shared__ struct *particles;
    ...
    if (planet == True) {   
		...
		float r_xz = sqrt(pow((particles[i].position.x + CENTER_MASS_X),2) + pow((particles[i].position.z + CENTER_MASS_Z),2));
...
{
  struct Particle *global_P = (struct Particle *)devP;
  struct Particle *global_A = (struct Particle *)devA;
...
__global__ void update_vel(void *devP, void *devA, void *devA_next)
{
  struct Particle *global_P = (struct Particle *)devP;
  struct Particle *global_A = (struct Particle *)devA;
  struct Particle *global_A_next = (struct Particle *)devA_next;

  int i= blockIdx.x*blockDim.x + threadIdx.x;
  
  global_P[i].velocity.x = global_P[i].velocity.x + (global_A_next[i].position.x + global_A[i].position.x) / 2.0f * Time_step; 
  global_P[i].velocity.y = global_P[i].velocity.y + (global_A_next[i].position.y + global_A[i].position.y) / 2.0f * Time_step; 
  global_P[i].velocity.z = global_P[i].velocity.z + (global_A_next[i].position.z + global_A[i].position.z) / 2.0f * Time_step; 
...
void launchkernel(Particle *cpuP,Particle *cpuA)
{
  ...
  // OpenGL rendering global_P position first

  // update global_P position
  ...
  // OpenGL rendering updated global_P position
